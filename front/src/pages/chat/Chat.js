...
export const Chat = () => {
    const [messages, setMessages] = useState([{text: 'Hello!', belongsToCurrentUser: false, isAudio: false}]);
    const [message, setMessage] = useState('');

    const processMessage = () => {
        setMessages([...messages, {text: message, belongsToCurrentUser: true, isAudio: false}]);
        setMessage('');
    }
    ...
    const handleSpeechRecognized = (receivedMessage) => {
        ...
        setMessages([...messages, {text: receivedMessage, belongsToCurrentUser: true, isAudio: true}]);
    }
            ...
            messages.map(message => {
                if(!message.isAudio) {
                    ...
                    return (
                        ...
                        <div className="rce-mbox-text right"><IconButton onClick={() => playMessage(message.text)}><PlayCircleIcon/></IconButton></div>
                        </div>
                        </div>
                        </div>
                    )
